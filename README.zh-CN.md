...
- 补强远端 `codex app-server` 会话处理，重连行为、fallback 诊断和远端 thread 选择日志都更完整。
- `SSH Target` 现在会结合本机 SSH 配置给出 host 建议，同时保留直接输入 `user@host` 的原有方式。
- hooks 安装、socket 处理、transcript watcher 和 JSONL 读取会显式记录失败原因，不再主要依赖静默吞错。
- 将聊天、parser、session store 和 tool result 代码拆成更聚焦的组件，降低后续维护风险。
- 继续补强仓库发布链路：加入 staged Swift 质量检查、CI 构建覆盖、启发式审计校准，以及 unsigned release fallback 打包。
